...
import { AlertCircle, Microscope, ShoppingCart, PackageOpen, Filter } from "lucide-react";
...
export default function Consumables() {
  ...
  // Filter for consumables category products
  const consumablesProducts = allProducts?.filter(p => p.category === 'consumables');

  // Get unique product types
  const productTypes = useMemo(() => {
    if (!consumablesProducts) return [];
    const types = new Set(consumablesProducts.map(p => p.productType).filter(Boolean));
  ...
  const products = useMemo(() => {
    if (!consumablesProducts) return [];
    if (selectedType === "all") return consumablesProducts;
    return consumablesProducts.filter(p => p.productType === selectedType);
  }, [consumablesProducts, selectedType]);
      ...
      <section className="container py-12">
        <div className="mb-8">
          <div className="flex items-center gap-3 mb-4">
            ...
            <h2 className="text-3xl font-bold text-white">Consumables</h2>
          </div>
          <p className="text-white/80">
            Essential lab supplies and consumable materials for your research
          </p>
        </div>
        ...
        {isLoading ? (
          ...
          <Card className="p-12">
            <div className="text-center">
              ...
              <h3 className="text-lg font-semibold mb-2">No Consumables Available</h3>
              <p className="text-muted-foreground mb-6">
                We're currently updating our consumables inventory. Please check back soon.
              </p>
              <Link href="/">
